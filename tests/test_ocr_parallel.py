...
def fake_pipeline_factory(*, pipeline, **kwargs):
    """Top-level picklable factory used by spawn-based unit tests."""
...
class ParallelOcrTests(unittest.TestCase):
    def test_spawn_workers_reuse_one_pipeline_per_process(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            factory_log = root / "factory.log"
            images.mkdir()

            for index in range(8):
                (images / f"page_{index:05d}.png").write_bytes(b"x")

            previous = os.environ.get("PADDLE_BATCH_OCR_TEST_FACTORY_LOG")
            os.environ["PADDLE_BATCH_OCR_TEST_FACTORY_LOG"] = str(factory_log)
            ...
            try:
                ...
                if previous is None:
                    os.environ.pop("PADDLE_BATCH_OCR_TEST_FACTORY_LOG", None)
                else:
                    os.environ["PADDLE_BATCH_OCR_TEST_FACTORY_LOG"] = previous

            self.assertEqual(result.success_count, 8)
            self.assertEqual(result.failed_count, 0)

            pids = factory_log.read_text(encoding="utf-8").splitlines()
            self.assertGreaterEqual(len(set(pids)), 1)
            self.assertLessEqual(len(set(pids)), 2)
            self.assertEqual(len(pids), len(set(pids)))

            for index in range(8):
                payload = json.loads(
                    (output / f"page_{index:05d}_result.json").read_text(
                        encoding="utf-8"
                    )
                )
    ...
    def test_output_name_collision_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            images = root / "images"
            images.mkdir()
            (images / "same.png").write_bytes(b"x")
            (images / "same.jpg").write_bytes(b"x")
