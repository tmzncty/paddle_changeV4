    ...
    try:
        ...
        with Pool(processes=num_processes) as pool:
            ...
            for i, result in enumerate(results):
                ...
                # Progress display (removed detailed output, only kept for PDF conversion)
...
def process_image(image_info):
    ...
    try:
        ...
        with RedirectStdout():  # Use the context manager
            start_ocr_time = time.time()  # Time the OCR process
        ...
        if output:  # Check if the output is not empty
            ...
            # No per-image output here
            return ocr_time, image_path  # Return OCR time and path

        else:
            # Handle cases where OCR returns no results, log to file
            colored_output(f"[{get_beijing_time()}] No OCR results for {image_path}", "yellow", log_file_path)
            return None, image_path  # Still return image_path for error tracking
...
def main():
    ...
    with get_context("spawn").Pool(
        ...
        # Count total number of images/PDF pages *once* at the beginning
        if num_images == 0:  # Calculate only once
            for root, _, files in os.walk(image_root_dir):
                for file in files:
                    if file.lower().endswith(".pdf"):
                        try:
                            with fitz.open(os.path.join(root, file)) as doc:
                                num_images += doc.page_count
                        except Exception as e:
                             colored_output(f"[{get_beijing_time()}] Error getting page count from {file}: {e}", "red", log_file_path)
        ...
        for pdf_path, output_dir, error_dir, log_file_path, _ in pdf_list:
            ...
            # 2. Prepare image_info
            image_info_list = [
                (image_path, output_dir, error_dir, log_file_path, False)  # is_pdf=False for images
                for image_path in image_paths
            ]

            # 3. Process the extracted images
            ocr_results = pool.imap_unordered(process_image, image_info_list)

            # Gather OCR results, calculate speed, and update progress
            for ocr_time, image_path in ocr_results:
                processed_count += 1
                if ocr_time is None:  # Handle errors
                    error_count += 1
                #else: # Removed per-image speed output
                #   speed = 1 / ocr_time if ocr_time > 0 else 0
                # Calculate remaining time and ETA
                elapsed_time = time.time() - start_time
                avg_speed = elapsed_time / processed_count if processed_count > 0 else 0  # overall average speed (seconds/page)
                remaining_pages = num_images - processed_count
                remaining_time = remaining_pages * avg_speed if avg_speed > 0 else 0
                ...
                # Print progress information *after each page* (dynamic update)
                print(
                    f"Processed: {processed_count}/{num_images} pages ({processed_count/num_images*100:.1f}%), "
                    f"Avg. Speed: {avg_speed:.4f} s/page, ETA: {eta} ({remaining_formatted}), Errors: {error_count}",
                    end="\r",
                    flush=True
                )
            ...
            # Newline after processing each PDF (for cleaner output)
            print()  # Ensure a newline after the dynamic progress
            colored_output(f"[{get_beijing_time()}] Finished processing PDF: {pdf_path}", "blue", log_file_path)
