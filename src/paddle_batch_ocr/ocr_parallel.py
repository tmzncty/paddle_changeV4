...
The parallel layer intentionally reuses the serial task/result contract. Each
...
from .ocr_runner import OcrBatchResult, OcrRunnerError, OcrTask, OcrTaskResult, _validate_existing_result
...
def _init_parallel_worker(config: ParallelWorkerConfig) -> None:
    ...
    _WORKER_STORE = ManifestStore(Path(config.manifest_path)) if config.manifest_path is not None else None
...
def _execute_parallel_task(task: OcrTask) -> OcrTaskResult:
    config = _WORKER_CONFIG
    if config is None:
        return OcrTaskResult(task.source, task.output_json, "failed", "OcrRunnerError: parallel OCR worker was not initialized")

    store = _WORKER_STORE
    try:
        if store is not None:
            store.mark_started(task.source, "ocr", worker=f"pid-{os.getpid()}", device=config.device)
        predict_one_to_json(
        ...
        if store is not None:
            store.mark_success(task.source, "ocr", result_path=task.output_json)
        return OcrTaskResult(task.source, task.output_json, "success")
    except Exception as exc:
        if store is not None:
            try:
                ...
                pass
        return OcrTaskResult(task.source, task.output_json, "failed", f"{type(exc).__name__}: {exc}")
    ...
    *,
    ...
    try:
        for index, task in enumerate(tasks):
            try:
                previous = store.get_job(task.source, "ocr") if store is not None else None
                needs_run = store.needs_run(task.source, "ocr") if store is not None else True
                if task.output_json.exists():
                    can_adopt = resume and (store is None or previous is None or not needs_run)
                    if can_adopt:
                        _validate_existing_result(task.output_json)
                        if store is not None:
                            store.mark_success(task.source, "ocr", result_path=task.output_json)
                        prepared[index] = OcrTaskResult(task.source, task.output_json, "skipped")
                        continue
                    if not overwrite:
                        reason = (
                            "existing result is stale according to manifest; use --overwrite"
                            if store is not None and previous is not None and needs_run
                ...
                if store is not None:
                    try:
                        ...
                        pass
                prepared[index] = OcrTaskResult(task.source, task.output_json, "failed", f"{type(exc).__name__}: {exc}")
    finally:
        if store is not None:
            store.close()
...
def run_ocr_parallel(
    ...
    *,
    ...
    """Execute already-discovered OCR tasks with one lazy pipeline per process."""
    ...
    if device != "cpu":
        raise OcrRunnerError(
            "workers > 1 currently requires explicit device='cpu'; GPU worker pools need an explicit device map and are not enabled yet"
        )
    if start_method != "spawn":
        raise OcrRunnerError("parallel OCR currently supports only the 'spawn' start method")

    pending, results = _prepare_tasks(tasks, manifest_path=manifest_path, resume=resume, overwrite=overwrite)
    if not pending:
        return OcrBatchResult(tasks=tuple(results[index] for index in range(len(tasks))))
    ...
    config = ParallelWorkerConfig(
        ...
        overwrite=overwrite,
        manifest_path=os.fspath(Path(manifest_path).expanduser()) if manifest_path is not None else None,
        create_pipeline_fn=create_pipeline_fn,
    )
    context = multiprocessing.get_context(start_method)
    max_workers = min(workers, len(pending))
    with ProcessPoolExecutor(max_workers=max_workers, mp_context=context, initializer=_init_parallel_worker, initargs=(config,)) as executor:
        future_map = {executor.submit(_execute_parallel_task, task): (index, task) for index, task in pending}
            ...
            try:
                ...
                if manifest_path is not None:
                    try:
                        ...
                        pass
                results[index] = OcrTaskResult(task.source, task.output_json, "failed", f"{type(exc).__name__}: {exc}")
