...
from .paddlex_adapter import PipelineRef, PaddleXResultError, create_ocr_pipeline, default_ocr_predict_kwargs, parse_paddlex_ocr_result, predict_one_to_json
...
def discover_ocr_tasks(input_path: Path, output_dir: Path) -> Tuple[OcrTask, ...]:
    ...
    if source.is_file():
        ...
        return (OcrTask(source, target_root / f"{source.stem}_result.json"),)
    ...
    for root, dirs, files in os.walk(source):
        ...
        for filename in files:
            ...
            tasks.append(OcrTask(resolved, target_root / output_relative))
    ...
    *,
    ...
    if workers > 1:
        ...
        return run_ocr_parallel(
            tasks, workers=workers, pipeline_ref=pipeline_ref, device=device,
            engine=engine, use_hpip=use_hpip, manifest_path=manifest_path,
            resume=resume, overwrite=overwrite, predict_kwargs=predict_kwargs,
            create_pipeline_fn=create_pipeline_fn, start_method="spawn",
        )
    ...
    def get_pipeline() -> object:
        ...
        try:
            pipeline = create_ocr_pipeline(pipeline_ref, device=device, engine=engine, use_hpip=use_hpip, create_pipeline_fn=create_pipeline_fn)
    ...
    try:
        for task in tasks:
            try:
                previous_record = store.get_job(task.source, "ocr") if store is not None else None
                manifest_needs_run = store.needs_run(task.source, "ocr") if store is not None else True
                if task.output_json.exists():
                    can_adopt_existing = resume and (store is None or previous_record is None or not manifest_needs_run)
                    if can_adopt_existing:
                        ...
                        results.append(OcrTaskResult(task.source, task.output_json, "skipped"))
                        continue
                    if not overwrite:
                        reason = (
                ...
                if store is not None:
                    store.mark_started(task.source, "ocr", worker=f"pid-{os.getpid()}", device=device or "auto")
                predict_one_to_json(get_pipeline(), task.source, task.output_json, overwrite=overwrite, predict_kwargs=predict_kwargs)
                if store is not None:
                    store.mark_success(task.source, "ocr", result_path=task.output_json)
                results.append(OcrTaskResult(task.source, task.output_json, "success"))
            except Exception as exc:
                if store is not None:
                    try:
                        ...
                        pass
                results.append(OcrTaskResult(task.source, task.output_json, "failed", f"{type(exc).__name__}: {exc}"))
    finally:
        if store is not None:
            store.close()
